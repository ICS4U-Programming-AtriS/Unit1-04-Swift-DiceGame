...
repeat {
    ...
    if let guess = Int(userInput) {
        if (guess < minNum || guess > maxNum) {
            ...
            print("\033[0;31mERROR: GUESS CAN'T BE OUTSIDE RANGE")
        } else if (guess > correctNum) {
            // Message if the user's guess is too high
            print("\033[0;34mYour guess is too high!")
        } else if (guess < correctNum) {
            // Message if the user's guess is too low
            print("\033[0;34mYour guess is too low!")
        } else {
            ...
            print("\033[0;32mYou got the number right!")
            // BREAK OUT OF THE LOOP
          break;
        }
    } else {
        // Error message for non-numeric guess. [IN RED]
        print("\033[0;31mERROR: GUESS MUST BE AN INTEGER")
    }
} while (true)
